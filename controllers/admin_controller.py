...
from models import Usuario, ProductoAlmacenad, Agenda,Cita
...
def eliminar_agenda(id):
    ...
    if agenda:
         citas_asociadas = Cita.query.filter_by(id_a=agenda.id_a).first()
    if citas_asociadas:
        flash('No se puede eliminar la agenda porque tiene citas registradas. Elimina las citas primero.', 'danger')
        return render_template('noagenda.html')
    else:

        try:
            # Intenta eliminar la agenda de la base de datos
            ...
            flash('No se puede eliminar la agenda', 'danger')
            return render_template('noagenda.html')
...
@admin.route('/consulta_citas_no_tomadas')

def consulta_citas_no_tomadas():
    # Verifica si el usuario actual es un administrador
    if session.get('tipo_usuario') != 'admin':
        flash('No tienes permisos para consultar citas', 'danger')
        return redirect(url_for('main.index'))

    # Consulta todas las citas no tomadas
    citas_no_tomadas = Cita.query.filter_by(estado_c='no tomada').all()

    # Renderiza la plantilla con todas las citas no tomadas obtenidas
    return render_template('consultar_citas_no_tomadas.html', citas=citas_no_tomadas)


@admin.route('/registro_cita', methods=['GET', 'POST'])
def registro_cita():
    ...
    if request.method == 'POST':
        ...
        if session.get('tipo_usuario') == 'admin':
            ...
            user_id = request.form['user_id']  # Agrega esta línea para obtener el user_id del formulario
            id_s = request.form['id_s']
            id_a = request.form['id_a']
            estado_c = request.form['estado_c']

            # Aquí puedes utilizar el user_id en la lógica que necesites

            # Redireccionar a la página de inicio después de registrar la cita
            return render_template('registrocitexi.html')
        else:
            flash('No tienes permisos para registrar citas', 'danger')
            return redirect(url_for('index'))

    # Si la solicitud es GET, simplemente renderiza el formulario de registro
    return render_template('formregistro_cita.html', user_id=user_id)
